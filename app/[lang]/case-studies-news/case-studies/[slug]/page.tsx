...
export default async function CaseStudyDetailPage({
  ...
  const query = `
    ...
    FROM articles
    WHERE ${slugColumn} = $1 AND category = 'Case Studies'
    LIMIT 1
  `;
  ...
  return (
    <main className="min-h-screen bg-[#07071C] text-white">
      ...
      <div className="mx-auto max-w-4xl px-6 py-32 md:px-12 lg:px-16">
        <div
          className={`text-sm font-semibold uppercase tracking-wider text-[#00FF85] ${isRtl ? "text-right" : "text-left"}`}
          dir={isRtl ? "rtl" : "ltr"}
        >
        ...
        <h1
          className={`mt-4 text-4xl font-bold leading-tight tracking-tight text-white md:text-5xl lg:text-6xl ${fontClass} ${
        ...
        <div
          className={`prose prose-invert prose-lg mt-12 max-w-none text-white/80 ${fontClass} ${
            isRtl ? "text-right" : "text-left"
          } prose-headings:font-bold prose-headings:text-white prose-a:text-[#00FF85] hover:prose-a:text-[#00cc6a] marker:text-[#00FF85]`}
          dir={isRtl ? "rtl" : "ltr"}
